...
public class VoteTopicServiceImpl extends BaseService<VoteTopic> implements TbDVoteTopicService {
    ...
    @Override
    public List<VoteTopic> findAllVoteTopinc(VoteTopic voteTopic, QueryRequest request) {
    ...
    public Tree<VoteTopic> getVoteTopicButtonTree() {
        ...
        List<VoteTopic> voteTopics = this.findAllVoteTopinc(new VoteTopic(),null);
        buildTrees(trees,voteTopics);
        return TreeUtils.build(trees);
    }

    private void buildTrees(List<Tree<VoteTopic>>trees,List<VoteTopic> voteTopics){
        voteTopics.forEach(vote ->{
